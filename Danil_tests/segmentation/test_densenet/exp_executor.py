...
class OptimizerBuilder:
    @staticmethod
    def build_optimizer(optimizer_info, ses):
        ...
        return build_dict[opt_type](params, ses)
    ...
    @staticmethod
    def __adam_optimizer(params, ses):
        lr = params['learning_rate']
        beta1 = params['beta1']
        beta2 = params['beta2']
        ...
        learning_rate = tf.train.cosine_decay_restarts(learning_rate=starter_learning_rate,
        ...
        #    global_step, 2500, 0.94, staircase=True)
...
# ------------------------------------EXPERIMENT LOOP-------------------------------------------------------------------
    ...
    def _run_focal_experiment(self, exp_params):
        ...
        optimizer, global_step = OptimizerBuilder.build_optimizer(opt_info, self._sess)
                ...
                if self.generator is None:
                    ...
                    if loss_type == LossType.FocalLoss:
                        sub_train_info = model.genfit_focal(
                            gamma=gamma, optimizer=optimizer, epochs=1, iterations=self.iterations, global_step=global_step
                        )
                    elif loss_type == LossType.MakiLoss:
...
def do_cv_experiment(Xtest, Ytest, experiment_folder, exp_params_file, gen):
    ...
    trainer.set_generator(gen, iterations=500) 
...
if __name__ == "__main__":
    ...
    #do_cv_experiment(Xtest, Ytest, 'test1', 'exp_params.json', gen_1)

    #Xtest, Ytest, gen_2 = get_data_cv2()
    #do_cv_experiment(Xtest, Ytest, 'test2', 'exp_params.json', gen_2)

    Xtest, Ytest, gen_3 = get_data_cv3()
    do_cv_experiment(Xtest, Ytest, 'test3', 'exp_params_201.json', gen_3)
